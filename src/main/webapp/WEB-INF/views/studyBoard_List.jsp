				...
				<table class="tbl_1">
					...
					<c:forEach items="${boardList}" var="board">
						<tr class="currentList subjectTH">
							...
							<td class="titleTable">
								<div class="subjectS">
									<a href="geStudytBoard.do?seq=${board.seq}">[${board.recruitment}]
								</div>  ${board.title }</a>
							</td>
				...
				success : function(ajaxData) { // ajaxData : 그냥 아무 변수명을 쓰면된다, 뭐 data라고 써도되고
					...
					$.each(	ajaxData['jsonList'], function(	index, item) { // 데이터 =item
						...
						let tagHtml = "<tr class='currentList subjectTH'>"
							...
							+ "<td class='titleTable'>"+ "<a href='geStudytBoard.do?seq="+ ajaxData['jsonList'][index]['seq']+ "'>"+ "<div class='subjectS'>["+ ajaxData['jsonList'][index]['recruitment']+ "] </div>"
					...
					$.each(	ajaxData, function(	index, item) {  // 데이터 =item
						console.log(index);
							...
							let tagHtml = "<tr class='currentList subjectTH'>"
								...
								+ "<td class='titleTable'>"+ "<a href='geStudytBoard.do?seq=" + ajaxData[index].seq + "'>"+ "<div class='subjectS'>["+ ajaxData[index].recruitment + "] </div>"
